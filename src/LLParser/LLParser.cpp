...
void LLParser::Shift(const size_t value)
{
...
void LLParser::Fold(std::string const& ruleName, const size_t ruleSize)
{
...
Action LLParser::GetCurrentAction()
{
	return m_table[m_stateStack.top()][GetCurrentSymbol()];
}

std::string LLParser::GetCurrentSymbol()
{
	if (m_foldStack.empty())
	{
